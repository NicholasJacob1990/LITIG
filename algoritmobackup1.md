...
class MatchmakingAlgorithm:
    ...
    async def rank(self, case: Case, lawyers: List[Lawyer], *, top_n: int = 5,
        ...
        """
        Classifica os advogados e mescla com recomendações patrocinadas.
        """
        start_time = time.time()
        logger.info(
            "ranking_started",
            extra=safe_json_dump({
                "case_id": case.id, "num_lawyers": len(lawyers), "preset": preset
            }),
        )

        # 4️⃣ Lógica de Gating / Boost para Casos Premium
        if case.is_premium:
            # Assumindo que 'created_at' está no objeto Case. Se não, precisa ser adicionado.
            ...
            if datetime.utcnow() < window_end:
                # GATING: Filtra apenas advogados PRO durante a janela exclusiva
                pro_lawyers = [l for l in lawyers if getattr(l, 'plan', 'FREE').upper() == "PRO"]
                if pro_lawyers:
                    logger.info("premium_gating_active", extra={"case_id": case.id, "pro_lawyers_found": len(pro_lawyers)})
                    lawyers = pro_lawyers
                else:
                    logger.warning("premium_gating_no_pro_available", extra={"case_id": case.id})
            else:
                # BOOST: Aplica um bônus para advogados PRO após a janela
                PRO_BONUS = 0.08  # Pode ser carregado da configuração
                for lawyer in lawyers:
                    if getattr(lawyer, 'plan', 'FREE').upper() == "PRO":
                        # O boost será aplicado ao score final posteriormente
                        # Vamos armazenar a intenção de boost
                        if 'pro_boost' not in lawyer.scores:
                             lawyer.scores['pro_boost'] = PRO_BONUS
                        logger.debug(f"pro_boost_bonus_set_for_{lawyer.id}", extra={"case_id": case.id})
        ...
        # Supondo que 'final_ranking' é a lista de advogados orgânicos classificados
        # E que 'top_n_lawyers' é a fatia dessa lista
        final_organic_lawyers = final_ranking[:top_n]

        # 5.3 Mesclar com advogados patrocinados
        organic_recommendations = []
        for lw in final_organic_lawyers:
            # Aplicar o pro_boost se existir
            fair_score = lw.scores.get('fair_base', 0.0)
            if 'pro_boost' in lw.scores:
                fair_score += lw.scores['pro_boost']
                fair_score = min(fair_score, 1.0)  # Garantir que não ultrapasse 1.0
            
            organic_recommendations.append(
                Recommendation(lawyer=lw, fair_score=fair_score, is_sponsored=False)
            )
        ...
        sponsored_recommendations = [
            Recommendation(
                lawyer=lw, 
                fair_score=lw.scores.get('fair_base', 0.0),  # Corrigido de 'fair' para 'fair_base'
                is_sponsored=True,
                ad_campaign_id=getattr(lw, 'ad_meta', {}).get('campaign_id')
            )
            for lw in sponsored_lawyers
        ]

        # Combina as listas
        combined_list = organic_recommendations + sponsored_recommendations
