...
class BusinessOpportunitySection extends BaseInfoSection {
      ...
      children: [
        ...
        buildInfoRow(
          ...
          trailing: competitorCount <= 2 
            ? buildStatusBadge(
        context,
        'Baixa Concorrência', backgroundColor: Colors.green.shade100)
            : buildStatusBadge(
        context,
        'Alta Concorrência', backgroundColor: Colors.red.shade100),
        ),
  ...
  String _getOpportunityRating() {
    ...
    final score = ((estimatedValue ?? 0.0) / 1000) * (successProbability ?? 0.0);
  ...
  String _getClientBudgetRange() {
    ...
    final maxBudget = (estimatedValue ?? 0.0) * rangeMultiplier;
    return '${formatCurrency(estimatedValue ?? 0.0)} - ${formatCurrency(maxBudget)}';
  }
  ...
  String _getClientHistory() {
    ...
    if (isNewClient == true) return 'Cliente novo';
  ...
  String _getUrgencyLevel() {
    ...
    if (urgency != null && urgency >= 8) return 'Muito urgente';
    if (urgency != null && urgency >= 6) return 'Urgente';
    if (urgency != null && urgency >= 4) return 'Normal';
    return 'Não urgente';
  }

  Color _getUrgencyColor(BuildContext context) {
    final urgency = getContextualValue<int>('urgency_level', 5);
    if (urgency != null && urgency >= 8) return Colors.red;
    if (urgency != null && urgency >= 6) return Colors.orange;
    return Theme.of(context).colorScheme.primary;
  }

  String _getResponseTime() {
    final hours = getContextualValue<int>('response_time_hours', 24);
    if (hours != null && hours <= 2) return '$hours horas (Urgente)';
    if (hours != null && hours <= 24) return '$hours horas';
    final days = hours != null ? (hours / 24).ceil() : 1;
    return '$days ${days == 1 ? 'dia' : 'dias'}';
  }

  Color _getResponseTimeColor(BuildContext context) {
    final hours = getContextualValue<int>('response_time_hours', 24);
    if (hours != null && hours <= 2) return Colors.red;
    if (hours != null && hours <= 12) return Colors.orange;
    return Theme.of(context).colorScheme.primary;
  }
  ...
  String _getCompetitiveAdvantage() {
    ...
    final indexValue = getContextualValue<int>('advantage_index', 0) ?? 0;
    final index = indexValue % advantages.length;
    return advantages[index];
  }

  int _calculateMargin() {
    final complexity = getContextualValue<int>('complexity_score', 7);
    return (30 - (complexity ?? 7) * 2).clamp(15, 45);
  }

  int _calculateROI() {
    final margin = _calculateMargin();
    final successProbability = getContextualValue<double>('success_probability', 0.85) ?? 0.85;
    return (margin * successProbability).round();
  }

  String _getPaymentTerm() {
    final terms = ['30 dias', '45 dias', '60 dias'];
    final indexValue = getContextualValue<int>('payment_term_index', 0) ?? 0;
    final index = indexValue % terms.length;
    return terms[index];
  }
  ...
  String _getAIInsight() {
    ...
    final indexValue = getContextualValue<int>('insight_index', 0) ?? 0;
    final index = indexValue % insights.length;
