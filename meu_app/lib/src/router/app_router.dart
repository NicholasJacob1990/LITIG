...
import 'package:meu_app/src/features/auth/presentation/screens/root_router_screen.dart';
...
GoRouter appRouter(AuthBloc authBloc) {
  return GoRouter(
    ...
    redirect: (BuildContext context, GoRouterState state) {
      ...
      final bool loggedIn = authBloc.state is auth_states.Authenticated;

      if (!loggedIn) {
        return loggingIn ? null : '/splash';
      }

      if (loggingIn) {
        // Redireciona para o dashboard após o login, que é a tela inicial para ambos os perfis.
        return '/dashboard';
      }
    ...
    routes: [
      GoRoute(
        path: '/',
        builder: (context, state) => const RootRouterScreen(),
      ),
      GoRoute(
        path: '/splash',
